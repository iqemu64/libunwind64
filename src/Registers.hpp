...
public:
    ...
    void            setJmpbuf(int *v)       { fJmpbuf = v; }
private:
...
{
    bzero(&fRegisters, sizeof(fRegisters));
    bzero(&fHalfVectorRegisters, sizeof(fHalfVectorRegisters));
}

void Registers_arm64::initWithQemuContext()
{
    if(!is_iqemu_available())
        return;
    void *context = iqemu_get_current_thread_context();
    if(context){
        //if there is arm64 context, initialize the X0-X31 registers, maybe should only init X19-X28.
        //callee saved float point registers not initialized.
        for(int i = 0; i < 32; i ++) {
            setRegister(UNW_ARM64_X0 + i, iqemu_get_context_register(context, IQEMU_REG_X0 + i));
        }
    }
}
...
void Registers_arm64::jumpto()
{
    ...
    void *context = iqemu_get_current_thread_context();
    for(int i = 0; i < 32; i ++) {
        iqemu_set_context_register(context, IQEMU_REG_X0 + i, getRegister(UNW_ARM64_X0 + i));
    }
    
    iqemu_set_context_register(context, IQEMU_REG_PC, getIP());
    longjmp(fJmpbuf, 1);
}

void Registers_arm64::truncate_CFI(int count)
{
    if(is_iqemu_available())
        iqemu_truncate_CFI(count);
}
