...
static _Unwind_Reason_Code unwind_phase1(unw_context_t* uc, struct _Unwind_Exception* exception_object)
{
	...
	for (bool handlerNotFound = true; handlerNotFound; ) {
		...
		if ( frameInfo.handler != 0 ) {
            ...
            if(is_iqemu_available() && iqemu_need_emulation(p)) {
                iqemu_begin_emulation(p, __personality_routine, "iiiQ^v^v", personalityResult, 1, _UA_SEARCH_PHASE, exception_object->exception_class, exception_object,
...
#define dbgf printf
EXPORT _Unwind_Reason_Code _Unwind_CallStack(unw_context_t *uc, uint64_t *callstack, int *depth)
{
    unw_cursor_t cursor1;
    unw_init_local(&cursor1, uc);
    *depth = 0;
    ...
    // walk each frame
    for (int stepResult = 1;; stepResult = unw_step(&cursor1)) {
        if ( stepResult == 0 ) {
            dbgf("** unw_step reached bottom => _URC_END_OF_STACK\n");
            return _URC_END_OF_STACK;
        } else if ( stepResult < 0 ) {
            dbgf("** unw_step failed => _URC_FATAL_PHASE1_ERROR\n");
            return _URC_FATAL_PHASE1_ERROR;
        }
        ...
        unw_proc_info_t frameInfo;
        if ( unw_get_proc_info(&cursor1, &frameInfo) != UNW_ESUCCESS ) {
            dbgf("** unw_get_proc_info failed => _URC_FATAL_PHASE1_ERROR\n");
            return _URC_FATAL_PHASE1_ERROR;
        }
        ...
        unw_word_t pc;
        unw_get_reg(&cursor1, UNW_REG_IP, &pc);
        //dbgf("** pc = 0x%llX, start_ip = 0x%llX, func = %s, lsda = 0x%llX, personality = 0x%llX\n\n",
        //     pc, frameInfo.start_ip, functionName, frameInfo.lsda, frameInfo.handler);
        *callstack = pc;
        ++callstack;
        *depth = *depth + 1;
    }
...
EXPORT _Unwind_Reason_Code _Unwind_CallStack2(unw_context_t *uc, uint64_t *callstack, int depth)
{
    unw_cursor_t cursor1;
    unw_init_local(&cursor1, uc);
...
static _Unwind_Reason_Code unwind_phase2(unw_context_t* uc, struct _Unwind_Exception* exception_object)
{
	...
	while ( true ) {
		...
		if ( frameInfo.handler != 0 ) {
            ...
            _Unwind_Reason_Code personalityResult;
            if(is_iqemu_available() && iqemu_need_emulation(p)) {
                iqemu_begin_emulation(p, __personality_routine, "iiiQ^v^v",
                                      personalityResult, 1, action,
                                      exception_object->exception_class, exception_object,
                                      (struct _Unwind_Context*)(&cursor2));
            } else {
                personalityResult = (*p)(1, action,
						exception_object->exception_class, exception_object, 
						(struct _Unwind_Context*)(&cursor2));
            }
			switch ( personalityResult ) {
